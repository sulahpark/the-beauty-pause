...
const TBL_BRAND_APPLICATIONS = "tblHCD0gT94LDCJu4";
...
function LuckyDrawScreen({ spot, salon, product, lang, setLang, spotId, onBack, SS }) {
  ...
  const [step, setStep] = useState("info"); // info | already | success
  ...
  const submitEntry = async () => {
    ...
    if (!screenshot) {
      setError(lang==="fr"?"Merci de joindre une photo du résultat.":"Please attach a photo of your result.");
      return;
    }
    setSubmitting(true); setError("");
    try {
      if (localStorage.getItem(storageKey)) { setStep("already"); setSubmitting(false); return; }
      const formula = encodeURIComponent(`AND({spot_id}='${spotId}',{phone}='${phone.trim()}')`);
      const checkRes = await fetch(`https://api.airtable.com/v0/${AT_BASE}/${TBL_LUCKY_DRAW}?filterByFormula=${formula}`, {
        headers: { Authorization: `Bearer ${AT_KEY}` }
      });
      ...
      // upload the photo to get a hosted URL Airtable can attach
      // (requires a small serverless endpoint — see note)
      let screenshotUrl = null;
      try {
        const fd = new FormData();
        fd.append("file", screenshot);
        const upRes = await fetch("/api/upload-image", { method: "POST", body: fd });
        if (upRes.ok) { const upData = await upRes.json(); screenshotUrl = upData?.url || null; }
      ...
      if (screenshotUrl) fields.submission_photo = [{ url: screenshotUrl }];
      ...
      if (res.ok) {
        ...
        console.error("Lucky draw error:", data);
        setError(data?.error?.message || (lang==="fr"?"Une erreur s'est produite.":"Something went wrong."));
      }
        ...
        {step==="info" && <>
          <div style={{textAlign:"center",marginBottom:24}}>
            ...
            <h1 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"clamp(26px,6vw,34px)",fontWeight:400,color:"#1a1a1a",margin:"0 0 8px",lineHeight:1.2}}>
              {lang==="fr"?"Partagez votre résultat, gagnez un cadeau":"Share your result, get a reward"}
            </h1>
            <p style={{...SS,fontSize:"13px",color:"#888",margin:0,lineHeight:1.6}}>
              {lang==="fr"?"Une seule participation par personne.":"One entry per person."}
            </p>
          </div>
          ...
          <div style={{background:"#fff",border:"1px solid #ede8e2",borderRadius:16,padding:"20px",marginBottom:20}}>
            <div style={{display:"flex",gap:12,marginBottom:16,alignItems:"flex-start"}}>
              <div style={{width:24,height:24,borderRadius:"50%",background:"#1a1a1a",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,...SS,fontSize:"11px",fontWeight:700}}>1</div>
              <p style={{...SS,fontSize:"13px",fontWeight:600,color:"#1a1a1a",margin:0}}>{lang==="fr"?"Recevez le service au salon":"Get the service done at the salon"}</p>
            </div>
            <div style={{display:"flex",gap:12,marginBottom:16,alignItems:"flex-start"}}>
              <div style={{width:24,height:24,borderRadius:"50%",background:"#1a1a1a",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,...SS,fontSize:"11px",fontWeight:700}}>2</div>
              <p style={{...SS,fontSize:"13px",fontWeight:600,color:"#1a1a1a",margin:0}}>{lang==="fr"?"Prenez une photo du résultat":"Take a photo of the result"}</p>
            </div>
            <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
              <div style={{width:24,height:24,borderRadius:"50%",background:"#1a1a1a",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,...SS,fontSize:"11px",fontWeight:700}}>3</div>
              <p style={{...SS,fontSize:"13px",fontWeight:600,color:"#1a1a1a",margin:0}}>{lang==="fr"?"Envoyez-la ci-dessous":"Upload it below"}</p>
            </div>
          </div>
          ...
          <div style={{background:"#fff",border:"1px solid #ede8e2",borderRadius:16,padding:"20px",marginBottom:20}}>
            ...
            <label style={{...SS,fontSize:"10px",color:"#aaa",letterSpacing:"1.5px",textTransform:"uppercase",display:"block",marginBottom:5}}>
              {lang==="fr"?"Photo du résultat *":"Photo of your result *"}
            </label>
            {screenshotPreview ? (
              <div style={{position:"relative",marginBottom:8}}>
                <img src={screenshotPreview} alt="" style={{width:"100%",maxHeight:220,objectFit:"cover",borderRadius:8,border:"1px solid #ede8e2",display:"block"}}/>
                <button onClick={()=>{setScreenshot(null);setScreenshotPreview(null);}}
                  style={{position:"absolute",top:8,right:8,width:26,height:26,borderRadius:"50%",background:"rgba(0,0,0,0.6)",color:"#fff",border:"none",cursor:"pointer",fontSize:"14px"}}>×</button>
              </div>
            ) : (
              <label style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:6,padding:"24px",border:"1.5px dashed #ddd",borderRadius:10,cursor:"pointer",marginBottom:8}}>
                <span style={{fontSize:"22px"}}>📸</span>
                <span style={{...SS,fontSize:"12px",color:"#999"}}>{lang==="fr"?"Choisir une photo":"Choose a photo"}</span>
                <input type="file" accept="image/*" onChange={onPickScreenshot} style={{display:"none"}}/>
              </label>
            )}
          ...
          <button onClick={submitEntry} disabled={submitting}
            ...
            {submitting?(lang==="fr"?"Envoi…":"Submitting…"):(lang==="fr"?"Envoyer mon avis →":"Submit my review →")}
          </button>
        </>}
...
function Badge({children}) {
  return <span style={{...BSS,fontSize:"12px",color:"#a07832",letterSpacing:"1.5px",textTransform:"uppercase",fontWeight:700,display:"inline-block",marginBottom:14}}>{children}</span>;
}
function Divider({align}) { return <div style={{width:40,height:3,background:"#c9a96e",borderRadius:2,margin:align==="left"?"14px 0 30px":"14px auto 30px"}}/>; }
...
function CheckItem({children, light}) {
  return (
    <div style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:12}}>
      ...
      <p style={{...BKR,fontSize:"15px",color:light?"rgba(255,255,255,0.65)":"#444",margin:0,lineHeight:1.6}}>{children}</p>
    </div>
  );
}
function ProgramItem({children, accent}) {
  return (
    <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:12}}>
      <span style={{color:accent||"#c9a96e",fontSize:"13px",flexShrink:0}}>✓</span>
      <p style={{...BKR,fontSize:"15px",color:"rgba(255,255,255,0.6)",margin:0}}>{children}</p>
    </div>
  );
}
...
function BrandsHomePage() {
  ...
  return (
    <>
      ...
      {/* HERO */}
      <section style={{background:"#0d0d0d",position:"relative",overflow:"hidden",padding:"110px clamp(24px,6vw,80px) 0",animation:"fadeUp 0.5s ease both"}}>
        ...
        <div style={{width:"100%",overflow:"hidden",lineHeight:0.75,pointerEvents:"none",userSelect:"none"}}>
          <p style={{...CG,fontSize:"clamp(90px,16vw,220px)",fontWeight:600,color:"rgba(255,255,255,0.06)",whiteSpace:"nowrap",margin:0,textAlign:"center"}}>BEAUTYPAUSE</p>
        </div>
      </section>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:20,marginBottom:32,textAlign:"left"}}>
            {lineup.map(({t,d,status,live,image})=>(
              <div key={t} onClick={()=>{navigate("/brands/program");window.scrollTo(0,0);}} style={{background:"#fff",border:"1px solid #ede8e2",borderRadius:16,overflow:"hidden",cursor:"pointer"}}>
                <div style={{aspectRatio:"5/3",background:"#1a1a1a",position:"relative"}}>
                  {image&&<img src={image} alt={t} style={{width:"100%",height:"100%",objectFit:"cover",display:"block"}}/>}
                  <span style={{position:"absolute",top:12,left:12,...SS,fontSize:10,fontWeight:700,letterSpacing:"0.5px",textTransform:"uppercase",padding:"4px 10px",borderRadius:20,color:live?"#0d0d0d":"#999",background:live?"#c9a96e":"rgba(255,255,255,0.85)"}}>{status}</span>
                </div>
                <div style={{padding:"20px 22px",textAlign:"center"}}>
                  {t.includes("×") ? (
                    <div style={{margin:"0 0 8px"}}>
                      {t.split("×").map(s=>s.trim()).map((part,i,arr)=>(
                        <Fragment key={i}>
                          <p style={{...KR,fontSize:"17px",fontWeight:700,color:"#1a1a1a",margin:0,lineHeight:1.4}}>{part}</p>
                          {i<arr.length-1&&<Star size={13}/>}
                        </Fragment>
                      ))}
                    </div>
                  ) : (
                  ...
                  <p style={{...KR,fontSize:"14px",color:"#777",lineHeight:1.7,margin:0}}>{d}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* SALON NETWORK — map + sliding name list */}
      <section style={{background:"#fff",padding:"80px 0 90px"}}>
        <div style={{maxWidth:880,margin:"0 auto",textAlign:"center",padding:"0 clamp(24px,6vw,80px)"}}>
...
function BrandsAboutPage() {
  ...
  return (
    <>
      ...
      {/* OPENING */}
      <section style={{background:"#0d0d0d",padding:"96px clamp(24px,6vw,80px) 72px",animation:"fadeUp 0.5s ease both"}}>
        <div style={{maxWidth:880,margin:"0 auto"}}>
          <Badge>✦ About TBP</Badge>
          <h1 style={{...KR,fontSize:"clamp(26px,4.2vw,38px)",fontWeight:700,color:"#f5f0eb",lineHeight:1.5,margin:"8px 0 24px"}}>
            K-뷰티가 파리의 살롱에서<br/>경험되는 방식
          </h1>
          <p style={{...KR,fontSize:"16px",color:"rgba(255,255,255,0.5)",lineHeight:1.8,maxWidth:600,margin:0}}>
      ...
      <section style={{background:"#fff",padding:"64px clamp(24px,6vw,80px)"}}>
        <div style={{maxWidth:880,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 200px",gap:48,alignItems:"center"}}>
          <div>
            ...
            <Divider align="left"/>
            <h2 style={{...KR,fontSize:"clamp(20px,2.6vw,26px)",fontWeight:700,color:"#1a1a1a",margin:"14px 0 20px"}}>왜 작은 브랜드인가</h2>
            ...
            <p style={{...KR,fontSize:"17px",color:"#1a1a1a",fontWeight:600,lineHeight:1.75,margin:0}}>
      ...
      <section style={{background:"#fff",padding:"64px clamp(24px,6vw,80px)"}}>
        <div style={{maxWidth:880,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 200px",gap:48,alignItems:"center"}}>
          <div>
            ...
            <Divider align="left"/>
            <h2 style={{...KR,fontSize:"clamp(20px,2.6vw,26px)",fontWeight:700,color:"#1a1a1a",margin:"14px 0 20px"}}>왜 살롱인가</h2>
            <p style={{...KR,fontSize:"16px",color:"#444",lineHeight:1.75,margin:"0 0 16px"}}>
              뷰티는 진열대 위에서 기억되지 않습니다. 사람들은 누군가의 추천을 듣고, 직접 사용해 보고, 변화를 경험한 뒤에야 브랜드를 기억합니다. 특히 헤어, 네일, 속눈썹, 피부 관리에서는 제품보다 경험이 먼저입니다.
            </p>
            <p style={{...KR,fontSize:"16px",color:"#444",lineHeight:1.75,margin:0}}>
      ...
      <section style={{background:"#fff",padding:"64px clamp(24px,6vw,80px)"}}>
        <div style={{maxWidth:880,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 200px",gap:48,alignItems:"center"}}>
          <div>
            ...
            <Divider align="left"/>
            <h2 style={{...KR,fontSize:"clamp(20px,2.6vw,26px)",fontWeight:700,color:"#1a1a1a",margin:"14px 0 20px"}}>우리는 왜 시작했나</h2>
            <p style={{...KR,fontSize:"16px",color:"#444",lineHeight:1.75,margin:"0 0 16px"}}>
      ...
      <section style={{background:"#fff",padding:"64px clamp(24px,6vw,90px)"}}>
        <div style={{maxWidth:880,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 200px",gap:48,alignItems:"center"}}>
          <div>
            ...
            <Divider align="left"/>
            <h2 style={{...KR,fontSize:"clamp(20px,2.6vw,26px)",fontWeight:700,color:"#1a1a1a",margin:"14px 0 20px"}}>우리가 믿는 것</h2>
            <p style={{...KR,fontSize:"16px",color:"#444",lineHeight:1.75,margin:"0 0 20px"}}>
...
function BrandsProgramPage() {
  ...
  return (
    <>
      ...
      <section style={{background:"#0d0d0d",padding:"88px clamp(24px,6vw,80px) 40px",animation:"fadeUp 0.5s ease both"}}>
        <div style={{maxWidth:880,margin:"0 auto",textAlign:"center"}}>
          ...
          <h1 style={{...KR,fontSize:"clamp(24px,3.8vw,36px)",fontWeight:700,color:"#f5f0eb",lineHeight:1.4,margin:"8px 0 20px"}}>
            오프라인 뷰티 경험 플랫폼<br/>파리 살롱에서 사용하는 브랜드가 되세요.
          </h1>
          <p style={{...KR,fontSize:"15px",color:"rgba(255,255,255,0.5)",lineHeight:1.9,maxWidth:620,margin:"0 auto"}}>
            TBP는 브랜드 제품을 파리 살롱과 연결해 실제 경험으로 만듭니다.
          </p>
        </div>
      </section>

      {/* (1) SIGNATURE — everything in one light section */}
      <section style={{background:"#fff",padding:"40px clamp(24px,6vw,80px) 90px"}}>
        <div style={{maxWidth:880,margin:"0 auto"}}>
          <Badge>✦ 시그니처</Badge>
          <h1 style={{...KR,fontSize:"clamp(24px,3.6vw,34px)",fontWeight:700,color:"#1a1a1a",margin:"8px 0 8px"}}>Signature</h1>
          <p style={{...KR,fontSize:"15px",color:"#999",margin:"0 0 40px"}}>브랜드 제품이 실제 살롱 서비스 안에 통합되는 프로그램입니다. 광고가 아니라, 살롱의 실제 사용입니다.</p>

          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:20,marginBottom:56}}>
            {programLineup.map(({t,d,status,live,image})=>(
              <div key={t} style={{background:"#faf7f4",border:"1px solid #ede8e2",borderRadius:16,overflow:"hidden"}}>
                <div style={{aspectRatio:"3/5",background:"#e8e3da",position:"relative"}}>
                  {image&&<img src={image} alt={t} style={{width:"100%",height:"100%",objectFit:"cover",display:"block"}}/>}
                  <span style={{position:"absolute",top:12,left:12,...SS,fontSize:10,fontWeight:700,letterSpacing:"0.5px",textTransform:"uppercase",padding:"4px 10px",borderRadius:20,color:live?"#0d0d0d":"#666",background:live?"#c9a96e":"rgba(255,255,255,0.85)"}}>{status}</span>
                </div>
                <div style={{padding:"20px 18px 20px",textAlign:"center"}}>
                  {t.includes("×") ? (
                    <div style={{margin:"0 0 8px"}}>
                      {t.split("×").map(s=>s.trim()).map((part,i,arr)=>(
                        <Fragment key={i}>
                          <p style={{...KR,fontSize:"16px",fontWeight:700,color:"#1a1a1a",margin:0,lineHeight:1.4}}>{part}</p>
                          {i<arr.length-1&&<Star size={13}/>}
                        </Fragment>
                      ))}
                    </div>
                  ) : (
                    <p style={{...KR,fontSize:"16px",fontWeight:700,color:"#1a1a1a",margin:"0 0 6px"}}>{t}</p>
                  )}
                  <p style={{...KR,fontSize:"13px",color:"#888",lineHeight:1.6,margin:0}}>{d}</p>
                </div>
              </div>
            ))}
          </div>

          <div style={{borderTop:"1px solid #ede8e2",paddingTop:40,marginBottom:40}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:0}}>
              {[
                ...
                <div key={n} style={{display:"grid",gridTemplateColumns:"70px 1fr",gap:"0 16px",padding:"18px 0",borderTop:i===0?"none":"1px solid #f0e9dc"}}>
                  <p style={{...SS,fontSize:11,color:"#a07832",fontWeight:700,margin:0}}>{n}</p>
                  <div>
                    <p style={{...KR,fontSize:14,fontWeight:700,color:"#1a1a1a",margin:"0 0 4px"}}>{t}</p>
                    <p style={{...KR,fontSize:13,color:"#777",lineHeight:1.7,margin:0}}>{d}</p>
                  </div>
                </div>
              ))}
              <div style={{display:"grid",gridTemplateColumns:"70px 1fr",gap:"0 16px",padding:"18px 0",borderTop:"1px solid #f0e9dc"}}>
                <p style={{...SS,fontSize:11,color:"#a07832",fontWeight:700,margin:0}}>3단계</p>
                <div>
                  <p style={{...KR,fontSize:14,fontWeight:700,color:"#1a1a1a",margin:"0 0 8px"}}>결과</p>
                  <p style={{...KR,fontSize:13,color:"#777",lineHeight:1.8,margin:"0 0 4px"}}>· 매칭 성공: 정식 프로그램 운영 시작</p>
                  <p style={{...KR,fontSize:13,color:"#777",lineHeight:1.8,margin:0}}>· 매칭 불발: 제품 시딩으로 마무리. 희망하는 경우 추가 매칭 시도 가능합니다.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{borderTop:"1px solid #ede8e2",paddingTop:40,display:"grid",gridTemplateColumns:"1fr 1fr",gap:40}}>
            <div>
              <span style={{...SS,fontSize:11,fontWeight:700,letterSpacing:"0.5px",textTransform:"uppercase",color:"#a07832",background:"rgba(160,120,50,0.08)",border:"1px solid #e8d9b8",padding:"5px 12px",borderRadius:20,display:"inline-block",marginBottom:16}}>브랜드가 얻는 것</span>
              <div style={{marginBottom:28}}>
                {[
                  "살롱 서비스에 실제로 통합되는 프로그램",
                  "TBP 웹사이트 및 소셜미디어 홍보",
                  "실제 서비스 사용 콘텐츠",
                  "살롱 및 고객 구매 연결",
                  "해외 수출 실적 (수출신고대행 선택 시)",
      ...
      {/* (2) DISCOVERY — everything in one dark section */}
      <section style={{background:"#0d0d0d",padding:"80px clamp(24px,6vw,80px) 90px"}}>
        <div style={{maxWidth:880,margin:"0 auto"}}>
          <Badge>✦ 디스커버리 참여하기</Badge>
          <h2 style={{...KR,fontSize:"clamp(24px,3.6vw,34px)",fontWeight:700,color:"#f5f0eb",margin:"8px 0 8px"}}>Discovery</h2>
          <p style={{...KR,fontSize:"15px",color:"rgba(255,255,255,0.4)",margin:"0 0 32px"}}>여러 살롱에서 브랜드와 제품을 소개하는 가벼운 시작입니다.</p>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8,marginBottom:56}}>
            {["/images/aurabeaute04.jpeg","/images/IMG_0189.jpeg","/images/IMG20260529105931.jpg","/images/IMG_0175.jpeg","/images/IMG_0183.jpeg"].map((src,i)=>(
              <div key={i} style={{borderRadius:10,overflow:"hidden",aspectRatio:"1",background:"#222"}}>
          ...
          <div style={{borderTop:"1px solid rgba(255,255,255,0.1)",paddingTop:40,marginBottom:40}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:0}}>
              {[
                ...
                <div key={n} style={{display:"grid",gridTemplateColumns:"70px 1fr",gap:"0 16px",padding:"18px 0",borderTop:i===0?"none":"1px solid rgba(255,255,255,0.08)"}}>
                  <p style={{...SS,fontSize:11,color:"#c9a96e",fontWeight:700,margin:0}}>{n}</p>
                  <div>
                    <p style={{...KR,fontSize:14,fontWeight:700,color:"#f5f0eb",margin:"0 0 4px"}}>{t}</p>
                    <p style={{...KR,fontSize:13,color:"rgba(255,255,255,0.5)",lineHeight:1.7,margin:0}}>{d}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{borderTop:"1px solid rgba(255,255,255,0.1)",paddingTop:40,display:"grid",gridTemplateColumns:"1fr 1fr",gap:40}}>
            <div>
              <span style={{...SS,fontSize:11,fontWeight:700,letterSpacing:"0.5px",textTransform:"uppercase",color:"#c9a96e",background:"rgba(201,169,110,0.12)",border:"1px solid rgba(201,169,110,0.3)",padding:"5px 12px",borderRadius:20,display:"inline-block",marginBottom:16}}>브랜드가 얻는 것</span>
              <div style={{marginBottom:28}}>
                {[
                  "다수 살롱 오프라인 노출",
                  "TBP 웹사이트 및 소셜미디어 홍보",
            ...
            <div>
              ...
              <div style={{padding:"12px 0",borderTop:"1px solid rgba(255,255,255,0.08)"}}>
                <p style={{...SS,fontSize:11,color:"rgba(255,255,255,0.4)",letterSpacing:"0.5px",textTransform:"uppercase",margin:"0 0 4px"}}>참가비</p>
      ...
      {/* (2.5) CPNP — for brands not yet registered */}
        ...
        <div style={{maxWidth:880,margin:"0 auto"}}>
          ...
          <div style={{display:"flex",flexDirection:"column"}}>
            {[
              ...
              {q:"제품 수량 제한이 있나요?",a:"디스커버리는 20개 기준으로 운영됩니다. 시그니처는 신청 시 샘플 5개로 시작하며, 매칭이 확정되면 살롱 운영 기간과 서비스 횟수에 따라 필요 수량을 함께 정합니다."},
              {q:"제품 판매도 가능한가요?",a:"네, 가능합니다. QR을 통한 온라인 구매 연결뿐 아니라, 시그니처의 경우 살롱 현장 판매도 함께 진행할 수 있습니다."},
              {q:"CPNP 등록이 안 되어 있어도 참여할 수 있나요?",a:"시그니처는 유럽 내 사용 규정상 CPNP 등록이 필요합니다. 등록이 안 되어 있다면 TBP가 등록을 지원해드리며, 이 경우 별도 비용이 발생합니다."},
              {q:"신청하면 무조건 참여할 수 있나요?",a:"아니요. 브랜드 방향성과 제품 특성, 살롱과의 적합성을 검토한 후 참여 여부를 결정합니다. 모든 브랜드가 참여할 수 있는 건 아닙니다."},
            ].map(({q,a})=>(
              <div key={q} style={{padding:"22px 0",borderTop:"1px solid #ede8e2"}}>
                <p style={{...KR,fontSize:"15px",fontWeight:700,color:"#1a1a1a",margin:"0 0 8px"}}>{q}</p>
                <p style={{...KR,fontSize:"14px",color:"#666",lineHeight:1.8,margin:0}}>{a}</p>
              </div>
            ))}
...
function BrandsContactPage() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`https://api.airtable.com/v0/${AT_BASE}/${TBL_BRAND_APPLICATIONS}`, {
        method: "POST",
        headers: { Authorization: `Bearer ${AT_KEY}`, "Content-Type": "application/json" },
        body: JSON.stringify({ records: [{ fields }] })
      });
      ...
      if (res.ok) {
        ...
        console.error("Brand application save error:", data);
        // fall back to mailto so the submission isn't lost
        const body = [
          `이름: ${form.name}`, `소속팀 및 직함: ${form.title}`, `브랜드명: ${form.brand}`,
          `연락처: ${form.phone}`, `메일주소: ${form.email}`, `TBP를 알게 된 경로: ${form.source}`,
          `관심 있는 참여 방식: ${form.interest}`, `제품 카테고리: ${form.category}`,
        ].join("\n");
        ...
        <div style={{maxWidth:560,margin:"0 auto"}}>
          {sent ? (
            <div style={{textAlign:"center",padding:"40px 0"}}>
              ...
              <p style={{...KR,fontSize:"13px",color:"#888"}}>신청 내용이 접수되었어요. 검토 후 연락드릴게요.</p>
            </div>
          ) : (
            <form onSubmit={submit}>
              ...
              {error&&<p style={{...SS,fontSize:12,color:"#c0392b",marginBottom:14}}>{error}</p>}
              <button type="submit" disabled={submitting}
                style={{width:"100%",padding:"17px",background:submitting?"#ccc":"linear-gradient(135deg,#c9a96e,#b8944d)",color:"#0d0d0d",border:"none",borderRadius:12,cursor:submitting?"not-allowed":"pointer",...KR,fontSize:"15px",fontWeight:700,boxShadow:"0 6px 24px rgba(201,169,110,0.3)"}}>
                {submitting?"제출 중…":"프로그램 신청하기 →"}
              </button>
            </form>
          )}
        </div>
